  ...
  <div class="quant-page" @keydown.esc.window="closeGlossary">
    ...
    <template v-if="tab === 'factor'">
      <section class="factor-controls">
        ...
        <button class="primary run-button action-button" :disabled="loading" @click="runFactor">{{ loading ? '计算中' : '计算因子' }}</button>
      </section>
      ...
      <section class="factor-output">
        ...
        <ChartFrame
          class="result-table"
          title="因子时间序列"
          :description="`最近 ${factorRows.length} 条 · 支持横纵滚动查看`"
        >
          <DataGrid :rows="factorRows" :columns="factorColumns" :empty-text="loading ? '正在计算因子…' : '暂无结果'" />
        </ChartFrame>
      </section>
    </template>
