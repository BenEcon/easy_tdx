...
const roleOptions = [
  { value: 'user' as const, label: '标准用户', description: '管理自己的策略与偏好设置' },
  { value: 'admin' as const, label: '管理员', description: '可维护账户及系统数据' },
]
    ...
    <div class="admin-layout">
      <aside class="create-panel">
        ...
        <form @submit.prevent="addUser">
          ...
          <label class="role-field"><span>账户角色</span><MacSelect v-model="newRole" :options="roleOptions" aria-label="新账户角色" /></label>
          <button class="primary action-button create-account-button" type="submit" :disabled="creating">
            <svg class="button-icon" viewBox="0 0 20 20" aria-hidden="true"><circle cx="8" cy="7" r="3"/><path d="M3 16c.5-3 2.2-4.5 5-4.5 1.4 0 2.6.4 3.5 1.1M15 9v6M12 12h6"/></svg>
            <span>{{ creating ? '正在创建…' : '创建账户' }}</span>
          </button>
        </form>
      ...
      <main class="accounts-panel">
        ...
        <div v-else class="account-list">
          <article v-for="user in users" :key="user.id" :class="{ inactive: !user.active }">
            ...
            <div class="account-role-select"><MacSelect :model-value="user.role" :options="roleOptions" :disabled="busyId === user.id || !user.active" :aria-label="`${user.username} 的账户角色`" @update:model-value="changeRole(user, $event)" /></div>
...
.create-panel .create-account-button{width:100%;min-height:34px;margin-top:2px}.role-field :deep(.mac-select-trigger){min-height:38px}.account-role-select{width:124px}.account-role-select :deep(.mac-select-trigger){min-height:30px;padding:4px 8px;font-size:10px}.account-list article{grid-template-columns:38px minmax(130px,1fr) 80px 128px auto}@media(max-width:1050px){.account-list article{grid-template-columns:36px minmax(120px,1fr) 60px 120px auto}}
</style>
